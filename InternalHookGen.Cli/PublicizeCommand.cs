...
public sealed class PublicizeCommand : RootCommand
{
    public PublicizeCommand()
    {
        ...
        Add(
            new Argument<FileSystemInfo[]>("input")
            {
        ...
        Add(
            new Option<HookGenTarget>(
                "--target",
                () => HookGenTarget.All,
                "Targets for publicizing"
            )
        );
        Add(
            new Option<bool>(
                "--publicize-compiler-generated",
                "Publicize compiler generated types and members"
            )
        );
        Add(
            new Option<bool>("--dont-add-attribute", "Skip injecting OriginalAttributes attribute")
        );
        Add(
            new Option<bool>("--strip", "Strips all method bodies by setting them to `throw null;`")
        );
        Add(
            new Option<bool>(
                "--strip-only",
                "Strips without publicizing, equivalent to `--target None --strip`"
            )
        );
        Add(
            new Option<bool>(
                new[] { "--overwrite", "-f" },
                "Overwrite existing files instead appending a postfix"
            )
        );
    ...
    private static void Handle(
        ...
        string? output,
        HookGenTarget target,
        bool publicizeCompilerGenerated,
        bool dontAddAttribute,
        bool strip,
        bool stripOnly,
        bool overwrite,
    ...
    )
    {
        ...
        Log.Information(
            "Publicizing {Count} assemblies {Assemblies}",
            assemblies.Count,
            assemblies.Select(x => x.Name)
        );
